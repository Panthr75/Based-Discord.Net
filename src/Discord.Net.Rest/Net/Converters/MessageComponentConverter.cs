...
internal sealed class MessageComponentConverter : JsonConverter<IMessageComponent>
{
    ...
    public override IMessageComponent? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        var ob = JsonNode.Parse(ref reader, nodeOptions: new JsonNodeOptions()
        {
        ...
        var type = 0;
        if (ob.TryGetPropertyValue("type", out var typeValue))
        {
