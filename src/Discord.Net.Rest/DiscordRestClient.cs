...
namespace Discord.Rest
{
    ...
    public class DiscordRestClient : BaseDiscordClient, IDiscordClient
    {
        ...
        /// </summary>
        public new RestSelfUser CurrentUser { get => (RestSelfUser)base.CurrentUser; internal set => base.CurrentUser = value; }
        ...
        private static API.DiscordRestApiClient CreateApiClient(DiscordRestConfig config)
            => new API.DiscordRestApiClient(config.RestClientProvider, DiscordRestConfig.UserAgent, serializerOptions: SerializerOptions, useSystemClock: config.UseSystemClock, defaultRatelimitCallback: config.DefaultRatelimitCallback);
        ...
        /// <param name="doApiCallOnCreation">Optional function that returns whether an api call should be made on creation</param>
        /// <returns>
        ...
        public async Task<RestInteraction> ParseHttpInteractionAsync(string publicKey, string signature, string timestamp, byte[] body, Func<InteractionProperties, bool>? doApiCallOnCreation = null)
        {
            ...
            var model = JsonSerializer.Deserialize<API.Interaction>(body, SerializerOptions)!;

            return await RestInteraction.CreateAsync(this,
                model,
                doApiCallOnCreation is not null ?
                    doApiCallOnCreation(new InteractionProperties(model)) :
                    _apiOnCreation
                ) ?? throw new InvalidOperationException("Invalid interaction body!");
        }
        ...
        public async Task<RestSelfUser> GetCurrentUserAsync(RequestOptions? options = null)
        {
            ...
            {
                this.CurrentUser = RestSelfUser.Create(this, user);
            }
            else
            {
        ...
        [MemberNotNull(nameof(_applicationInfo))]
        public async Task<RestApplication> GetApplicationInfoAsync(RequestOptions? options = null)
        {
            if (this._applicationInfo == null)
            {
#pragma warning disable CS8774 // Member must have a non-null value when exiting. (It will)
                this._applicationInfo = await ClientHelper.GetApplicationInfoAsync(this, options).ConfigureAwait(false);
#pragma warning restore CS8774 // Member must have a non-null value when exiting.
            }
            return this._applicationInfo;
        }

        [MemberNotNull(nameof(_currentBotApplication))]
        public async Task<RestApplication> GetCurrentBotInfoAsync(RequestOptions? options = null)
        {
            if (this._currentBotApplication == null)
            {
#pragma warning disable CS8774 // Member must have a non-null value when exiting. (It will)
                this._currentBotApplication = await ClientHelper.GetCurrentBotApplicationAsync(this, options);
#pragma warning restore CS8774 // Member must have a non-null value when exiting.
            }
            return this._currentBotApplication;
        }
        ...
        Task IDiscordClient.DeleteTestEntitlementAsync(ulong entitlementId, RequestOptions? options)
            => DeleteTestEntitlementAsync(entitlementId, options);
        ...
        IAsyncEnumerable<IReadOnlyCollection<IEntitlement>> IDiscordClient.GetEntitlementsAsync(int? limit, ulong? afterId, ulong? beforeId,
            bool excludeEnded, ulong? guildId, ulong? userId, ulong[]? skuIds, RequestOptions? options) => GetEntitlementsAsync(limit, afterId, beforeId, excludeEnded, guildId, userId, skuIds, options);

        Task<IReadOnlyCollection<SKU>> IDiscordClient.GetSKUsAsync(RequestOptions? options) => GetSKUsAsync(options);
        ...
        async Task<IReadOnlyCollection<IPrivateChannel>> IDiscordClient.GetPrivateChannelsAsync(CacheMode mode, RequestOptions? options)
        {
            if (mode == CacheMode.AllowDownload)
                ...
                return ImmutableArray<IPrivateChannel>.Empty;
        }
        /// <inheritdoc />
        async Task<IReadOnlyCollection<IDMChannel>> IDiscordClient.GetDMChannelsAsync(CacheMode mode, RequestOptions? options)
        {
            if (mode == CacheMode.AllowDownload)
                ...
                return ImmutableArray<IDMChannel>.Empty;
        }
        /// <inheritdoc />
        async Task<IReadOnlyCollection<IGroupChannel>> IDiscordClient.GetGroupChannelsAsync(CacheMode mode, RequestOptions? options)
        {
            if (mode == CacheMode.AllowDownload)
                ...
                return ImmutableArray<IGroupChannel>.Empty;
        }
        ...
        async Task<IReadOnlyCollection<IGuild>> IDiscordClient.GetGuildsAsync(CacheMode mode, RequestOptions? options)
        {
            if (mode == CacheMode.AllowDownload)
                ...
                return ImmutableArray<IGuild>.Empty;
        }
        /// <inheritdoc />
